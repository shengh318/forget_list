  ...
  return (
    <section className="val-game" aria-label="Cupid Catch mini game">
      <div className="val-game-header">
        <h3><span className="val-game-title-icon">🏹</span> Cupid Catch</h3>
      ...
      <div className="val-game-stats">
        <span className="val-game-stat">
          <span className="stat-icon">💘</span> {score}
        </span>
        <span className="val-game-stat">
          <span className="stat-icon">🏆</span> {bestScore}
        </span>
        <span className={`val-game-stat ${timeLeft <= 3 && running ? "stat-urgent" : ""}`}>
          <span className="stat-icon">⏱️</span> {timeLeft}s
        </span>
      </div>

      <div className="val-game-bar-wrap">
        <div className="val-game-bar">
          <div className="val-game-fill" style={{ width: `${percent}%` }} />
        </div>
        <span className="val-game-bar-label">{score}/{goal}</span>
      </div>

      <div className="val-game-board" ref={boardRef}>
        <div className="val-game-bg-hearts">
          <span>💕</span><span>💖</span><span>💗</span>
        </div>
        <button
          type="button"
          className={`val-game-heart ${pop ? "val-game-heart-pop" : ""}`}
        ...
        {!running && (
          <div className="val-game-overlay">
            <span className="val-game-overlay-icon">{won ? "💘" : "💔"}</span>
            <span className="val-game-overlay-text">{won ? "Love wins!" : "Game over"}</span>
          </div>
        )}
      </div>

      <p className="val-game-goal">Goal: catch {goal} hearts before time runs out.</p>
    </section>
  );
}
